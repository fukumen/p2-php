...
if ($ng_type & self::HIGHLIGHT_CHAIN) {
	...
	$msg = preg_replace("(((?:&gt;|＞|-)+)($pattern_chain_nums))", "<span style=\"{$live_highlight_chain_style}\">$1$2</span>", $msg);
}
...
if ($ng_type & self::HIGHLIGHT_MSG) {
	foreach ($highlight_msgs as $h_msg) {
		if (preg_match('{^(?:<(regex|regex:i|regexi|i)>)?(?:<bbs>(.+?)</bbs>)?(?:<title>(.+?)</title>)?(.+)$}s', $h_msg, $matches)) {
			...
			if ($ignore_case) {
				$msg = mb_eregi_replace($pattern, "<span style=\"{$live_highlight_style}\">\\1</span>", $msg);
			} else {
				$msg = mb_ereg_replace($pattern, "<span style=\"{$live_highlight_style}\">\\1</span>", $msg);
			}
		}
	}
}
...
if ($ng_type & self::HIGHLIGHT_NAME) {
	...
	if ($_conf['ngaborn_watchoi4']) {
		...
		$name = preg_replace($pattern, "($1<span style=\"{$live_highlight_style}\">$2</span>$3)", $name);
	} else {
		$pattern = '#\(((?:[^\s()]+\s+)?(?:[0-9A-Za-z./*+]{4}-[0-9A-Za-z./*+]{4})(?:\s+\[.+])?)\)#';
		$name = preg_replace($pattern, "(<span style=\"{$live_highlight_style}\">$1</span>)", $name);
	}
}
