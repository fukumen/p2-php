...
|
  (?P<id>ID:[ ]?([0-9A-Za-z/.+]{8,14})(?=[^0-9A-Za-z/.+]|$)) # ID（8,10桁 +PC/携帯識別フラグ）
     ...
     * @var int
     */
    protected $_idcount_average;
    protected $_idcount_tops;
    protected $_watchoicount_average;
    protected $_watchoicount_tops;

    /**
     * IDとワッチョイに色を付ける
     * coloredid.coloring.type=0時のID、ワッチョイ毎のキャッシュ
     *
     * @var array
     */
    protected $idstyles = array();
    protected $watchoistyles = array();
    ...
    public function replaceBeId($date_id, $i)
    {
        ...
        $beid_replace = "<a href=\"https://be.{$_conf['2ch_domain']}/user/\$1\"{$_conf['ext_win_target_at']}>Lv.\$2</a>";
        ...
        if (preg_match($be_match, $date_id)) {
            ...
            $beid_replace = "<a href=\"https://be.{$_conf['2ch_domain']}/user/\$1\"{$_conf['ext_win_target_at']}>?\$2</a>";
    ...
    {
        if (array_key_exists('url', $_GET) && !empty($s['url'])) {
     ...
     * @return  string
     */
    protected function _coloredWatchoiStr($watchoi_id, $wid, $wid32, $classed = false)
    {
        global $_conf;
        ...
        if ($classed) {
            return $this->_coloredWatchoiStrClassed($watchoi_id, $wid32);
        }
        ...
        if ($rate > 1 && $this->thread->watchoicount[$wid] >= $rate) {
            switch ($_conf['coloredid.coloring.type']) {
            case 0:
                return $this->_coloredWatchoiStr0($watchoi_id, $wid, $wid32);
                break;
            case 1:
                return $this->_coloredWatchoiStr1($watchoi_id, $wid, $wid32);
                break;
            default:
            return "({$watchoi_id})";
            }
    ...
    private function _coloredWatchoiStrClassed($watchoi_id, $wid32)
    {
        $ret = '(<span class="' . self::cssClassedWatchoi($wid32) . '-b">' . $watchoi_id . '</span>)';
     ...
     * ワッチョイカラー オリジナル着色用
     */
    private function _coloredWatchoiStr0($watchoi_id, $wid, $wid32)
    {
        if (!function_exists('coloredIdStyle0')) {
            require P2_LIB_DIR . '/color/coloredIdStyle0.inc.php';
        }
        ...
        if ($colored) {
            $ret = "(<span style=\"{$colored}\">{$watchoi_id}</span>)";
        } else {
            $ret = "({$watchoi_id})";
        }
     ...
     * ワッチョイカラー thermon版用
     */
    private function _coloredWatchoiStr1($watchoi_id, $wid, $wid32)
    {
        if (!function_exists('coloredIdStyle')) {
            require P2_LIB_DIR . '/color/coloredIdStyle.inc.php';
        }
    ...
    public function getIdColorJs()
    {
        ...
        if (isset($this->thread->idcount) && count($this->thread->idcount) > 0) {
            ...
            $uline = ($this instanceof ShowThreadPC && $STYLE['a_underline_none'] != 1)
                ? 'idCol.colorStyle["textDecoration"] = "underline"' : '';
            $setupSPM = ($this instanceof ShowThreadPC) ? "idCol.setupSPMPC('{$this->spmObjName}');" : '';
...
EOJS;
        ...
        if (isset($this->thread->watchoicount) && count($this->thread->watchoicount) > 0) {
            ...
            $w_uline = ($this instanceof ShowThreadPC && $STYLE['a_underline_none'] != 1)
                ? 'watchoiCol.colorStyle["textDecoration"] = "underline"' : '';
            $setupSPM = ($this instanceof ShowThreadPC) ? "watchoiCol.setupSPMPC('{$this->spmObjName}');" : '';
