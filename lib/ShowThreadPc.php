...
EOJS;
    ...
    public function qRes($ares, $i)
    {
        ...
        if ($_conf['expack.spm.enabled']) {
            $js_date = $raw_date ? "'$raw_date'" : 'null';
            $has_watchoi = isset($this->thread->watchois[$i]) ? 'true' : 'false';
            $spmeh = " onmouseover=\"{$this->spmObjName}.show({$i},'{$qmsg_id}',event,{$js_date},{$has_watchoi})\"";
            $spmeh .= " onmouseout=\"{$this->spmObjName}.hide(event)\"";
        } else {
            $spmeh = '';
        }
...
EOP;
    ...
    public function watchoiFilter($watchoi_str, $watchoi_id)
    {
        ...
        if ($_conf['iframe_popup']) {
            return self::iframePopup($filter_url, $watchoi_str, $_conf['bbs_win_target_at']) . $num_ht;
        }
        return "<a href=\"{$filter_url}\"{$_conf['bbs_win_target_at']}>{$watchoi_str}</a>{$num_ht}";
    }
